...
def main() -> int:
    ...
    ax = axes[0]
    pols = ["green", "growth", "random"]
    pol_label = {"green": "green\n(decarbonise)", "growth": "growth\n(output first)",
                 "random": "mixed\n(random)"}
    ...
    ax.set_xlim(0, 100)
    ax.set_title(f"(a) Outcome by policy archetype (n={summ['all']['runs']})")
    ax.legend(loc="upper center", bbox_to_anchor=(0.5, -0.22), ncols=3, fontsize=7.5,
    ...
    ax.bar([f"by {yr}" for yr in years], vals, color="#b2182b", width=0.62)
    ax.set_ylabel("share of losses, %")
    ax.set_title("(b) When losing runs fail")
    for i, v in enumerate(vals):
        ax.text(i, v + 0.8, f"{v:.0f}%", ha="center", fontsize=8, color="#1a1a1a")
