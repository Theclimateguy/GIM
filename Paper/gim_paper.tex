...
The novelty of this work lies not in any single block but in the integration and the breadth of coupling. Integrated assessment models (IAM) such as DICE and FUND and large modeling frameworks (GCAM, MESSAGEix, REMIND) couple the economy and climate in detail but contain no sovereign finance, interstate relations, or
...
The model represents the world as a system of interacting countries evolving year by year. The state of
...
characteristic timescales; other greenhouse gases are accounted for --- their combined forward
forcing follows the SSP2-4.5 marker pathway (a plateau of about $0.73$~W/m$^2$ by 2100, rather than
an unbounded extrapolation) --- and the resulting warming is
...
ton at a two-century horizon --- within the broad modern range, below the central EPA-2023 and
RFF-SP estimates (about \$190) \citep{rennert2022,epa2023}; under Nordhaus-style discounting it is
about \$22, \$43, and \$46 at the 30-, 100-, and 200-year horizons (the 30-year value truncates the
long-run damage tail --- the honest horizon comparison). Importantly, this quantity is
\emph{genuinely} sensitive to the structure of future growth: across different calibrations of the
economic core it varies over a range of roughly \$90--280, and the development-convergence
recalibration of the growth core (faster catch-up growth, which makes the future richer and
discounts its damages more heavily) moved the central value down by about a quarter. Fed
DICE-2016R2's own, lower damage coefficient our pulse engine returns about \$15 under this faster
growth --- below DICE's \$31 --- so DICE underestimates damages relative to the empirically
calibrated function we adopt. We report these values together with their sensitivity rather than
tuning to a target; that sensitivity is itself a standalone result.
...
trillion USD. At the ten-year horizon its median is about $137$ trillion USD, with an interquartile
range of roughly $135$--$140$ and a $5$--$95$th-percentile interval of roughly $132$--$143$; the
...
v18.1.0. The computational core is deterministic and
...
CINC-style component, validated against the post-2022 militarization record) have all been added. The residual limitations --- the fast-approximation climate block, the
