...
# ==================================================================================
def main():
    # [F-bench 2026-08-24] forward_init=True. The benchmark is a forward experiment, and
    # without this flag it runs in the configuration where the resource-price subsystem
    # saturates at its clamps (honesty note N1 above) -- which is why every shock here is
    # injected downstream of prices. Measured: with forward_init=True no resource price
    # touches a bound in 30 years (Paper/revision/results/e6_resource_price_pinning.json),
    # so note N1 no longer applies and the headline doses move -- the oil-burden threshold
    # for the first importers to tip goes from ~3% to ~6% of GDP
    # (Paper/revision/results/e7_transfer_functions.json).
    base = load_world(max_agents=MAX_AGENTS, forward_init=True)
