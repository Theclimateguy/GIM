#!/usr/bin/env bash
# Assemble the unsigned GIM2.app bundle from the SwiftPM executable (no Xcode).
...
cp ".build/release/GIM2App" "$APP/Contents/MacOS/GIM2"
cp "Resources/Info.plist" "$APP/Contents/Info.plist"

# Brand: reuse the v1 icon/glyph/logo; GIM17.icns → GIM2.icns (Info.plist icon name).
[[ -f "$BRAND/GIM17.icns" ]] && cp "$BRAND/GIM17.icns" "$APP/Contents/Resources/GIM2.icns"
