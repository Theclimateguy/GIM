...
**N1 — SUPERSEDED 2026-08-24.** This note held that resource prices saturate at their caps within
one forward year, and the whole benchmark injects shocks downstream of prices for that reason. It
described the `forward_init=False` configuration. With `forward_init=True` — the headline forward
configuration — no resource price touches a bound in 30 years. Three underlying defects were
also repaired since: recycled metals supply was added on top of a base-year figure that already
represented total supply; the price-damping buffer read the geological reserve rather than
tradeable inventory; and energy demand grew with neither population nor income. Resource prices
are now scored against observed World Bank commodity indices in the historical backtest.
Note that this benchmark still runs with `forward_init=False`, and the headline oil threshold
doubles (≈3% → ≈6% of GDP) under the forward configuration; rerun before quoting it.
The original note read: GIM's global resource-price subsystem
...
Three cross-sector channels on one panel, each normalized to its own peak, characterized by the
**shape and gain** of each transfer function: near-linear for A (log-log exponent ~1.2 in the tax
rate), genuinely threshold-stepped for B (five distinct levels, largest jump five countries),
power-law for C (exponent ~1.2).

**Framing corrected 2026-08-24.** This figure previously argued an "integration dividend" — a
positive GIM slope against a sectoral flat line at zero. That comparison does not work: a model
with no society block has a slope of zero on the society axis *by construction*, not by
measurement, so the contrast is a category statement rather than a benchmark. What the sectoral
anchors establish is that GIM reproduces the first-order effect; what the sweeps establish is the
*form* of the cross-sector response, and it is the variety of forms — not a nonzero slope — that
is the result.
