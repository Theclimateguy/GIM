...
5. **[Built — activation pending] Forward-looking expectations** for investment and saving. A
   near-rational (model-consistent, level-1) expectation operator now exists (E4.3,
   `docs/EXPECTATIONS.md`): agents forecast by running an H-step event-frozen projection of the model
   itself (reusing `step_world` under a recursion guard). **Both sites are wired** — investment sources
   its expected-growth signal, and the Phillips anchor blends in the model-consistent expected inflation
   (weight grounded on the hybrid-NKPC forward share, Galí–Gertler). Parameters are literature-anchored
   (the in-sample window can't identify forward-looking behaviour). Ships **switchable and off by
   default** (golden-safe; full activation keeps the backtest within band); headline activation is the
   next, separate decision. A contained, additive change — not the equilibrium/DSGE rewrite that full
   rational expectations would require.
