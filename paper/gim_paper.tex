...
passed through to prices, with a pass-through coefficient ($\lambda\approx0.027$) estimated from a
...
meta-analysis \citep{howard2017,burke2015}. That this difference resides in the damage function
rather than the valuation machinery is confirmed by a direct cross-model check: fed DICE's damage
coefficient and discounting, the model's own marginal-pulse engine recovers DICE-2016R2's social
cost of carbon (approximately \$31 per ton of carbon dioxide, converging to \$32--33 over a
multi-century horizon), so the higher headline cost of carbon follows entirely from the higher,
empirically grounded damage function. From the damage function, the social cost of carbon ---
...
Cost of carbon & EPA-2023, RFF-SP & published cross-check; DICE engine reproduction & ${\approx}\$140$/t$\,\mathrm{CO_2}$ (range $140$--$380$); recovers DICE-2016R2 \$32--33 under DICE inputs \\
...
A targeted diagnostic adds a spatial dimension to this block. Real interstate conflict is
overwhelmingly local: over 1990--2023 the two states share a land border in about $92\,\%$ of cases
(some $46\times$ the chance rate, median centroid distance $863$\,km), matching the
territorial-proximity regularity of conflict studies \citep{vasquez1995}. The model's relational
escalation rule, which targets the most hostile partner with no geographic term, is far less local
($2.7\times$ chance). Augmenting the conflict-risk score with a spatial lag of neighbouring countries'
risk (spatial contagion; \citealp{gleditsch2007, buhaug2008}) raises the ranking AUC from $0.772$ to
$0.805$ on the geographically matched subset --- a modest but genuine gain (Fig.~\ref{fig:geography}).
A switchable adjacency-contagion term was therefore added to the relational dynamics, off by default
so the validated trajectories are unchanged; enabling it lifts the locality of the model's own
escalation from $2.7\times$ to $9.6\times$ chance, closing part of the gap to reality.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig10_geography.pdf}
\caption{Geography of conflict. \emph{(a)}~Neighbour adjacency relative to chance: real interstate
conflict (UCDP 1990--2023) is strongly local, the model's escalation targets are not, and enabling
the switchable adjacency-contagion term moves the model toward reality (median dyad distances in km
annotated). \emph{(b)}~Leverage on skill: a spatial lag of neighbouring conflict risk raises the
conflict-ranking AUC from $0.772$ to $0.805$; the neighbour exposure alone already scores $0.690$.}
\label{fig:geography}
\end{figure}
