...
spending, technology diffusion through trade, and a conditional-convergence term in which poorer
economies catch up faster --- its slope ($0.0093$ per log unit of the per-capita output gap to the
frontier) fit to the 2015--2023 cross-section of real purchasing-power-parity output, so the model
reproduces the observed catch-up growth of China and India; for forward projections the drift is
anchored to the SSP2 socio-economic development scenario \citep{riahi2017}. The energy, resource, and capital markets clear through prices: energy demand and
...
carbon-intensity decline. Emissions accumulate in the atmosphere and decay over several
characteristic timescales; other greenhouse gases are accounted for, and the resulting warming is
computed by a standard two-reservoir surface--ocean heat-balance model. The block is calibrated to mainstream scientific assessment: it
...
meta-analysis \citep{howard2017,burke2015}. A direct cross-model check isolates where our headline
cost of carbon comes from: fed DICE's \emph{own} lower damage coefficient and discounting, the
model's marginal-pulse engine returns about \$20 per ton of carbon dioxide at a multi-century
horizon --- below DICE-2016R2's \$31. That gap is not an engine artefact but a growth effect: our
...
empirically grounded damage function (about two and a half times DICE's), and the sub-DICE value
under DICE's own damages indicates that DICE-2016R2 underestimates damages once realistic growth is
imposed. From the damage function, the social cost of carbon ---
...
product, world emissions, and temperature. With the corrected capital initialisation both the
Cobb--Douglas and the closed nested-substitution core reach a product error of about $0.62$ trillion
USD; the gain from the nested substitution function with market clearing now shows in emissions
(Fig.~\ref{fig:valid}a), where it lowers the error from $1.87$ to $0.93$ gigatons.
...
\midrule
Economy & PWT, WDI & retrospective RMSE, 2015--2023 & GDP $0.62$ trillion USD (Cobb--Douglas $0.62$) \\
Climate and carbon & IPCC AR6, GCB & retrospective RMSE, 2015--2023 & $\mathrm{CO_2}$ $0.93$ Gt; $T$ $0.135\,^\circ$C \\
Cost of carbon & EPA-2023, RFF-SP & published cross-check; Ramsey discounting & ${\approx}\$95$/t$\,\mathrm{CO_2}$ (range $95$--$280$); under DICE's lower damages SCC is ${\approx}\$20$, i.e.\ DICE-2016R2 underestimates damages relative to our empirically calibrated function \\
...
a near-zero pure rate of time preference it is roughly \$95 per ton --- within the broad modern
range, below the central EPA-2023 and RFF-SP estimates (about \$190) \citep{rennert2022,epa2023};
under Nordhaus-style discounting it is about \$42. Importantly, this quantity is \emph{genuinely}
sensitive to the structure of future growth: across different calibrations of the economic core it
varies over a range of roughly \$95--280, and the development-convergence recalibration of the growth
core (faster catch-up growth, which makes the future richer and discounts its damages more heavily)
moved the central value down by about a quarter. Fed DICE-2016R2's own, lower damage coefficient our
pulse engine returns about \$20 under this faster growth --- below DICE's \$31 --- so DICE
underestimates damages relative to the empirically calibrated function we adopt. We report these
values together with their sensitivity rather than tuning to a target; that sensitivity is itself a
standalone result.
...
with the reference run is $\rho\approx0.99$ for the three physical quantities (in the range
...
(emissions scale, decarbonization rate, and land use for emissions; capital depreciation, the damage
coefficient, and energy intensity for product), while the lower-ranked parameters sit close together
and may exchange places. Social tension, by contrast, is almost insensitive to these physico-economic
...
($\rho\approx0.93$ at $8\!\to\!32$ trajectories, versus $\approx0.99$ for the physical quantities). Importantly,
...
$57$ modeled actors} --- the $50$ largest economies, residual regional aggregates (all remaining
countries of each region), and Taiwan --- which together cover essentially all of global output; in
the $2023$ base year (the validated canon) it is about $107$
trillion USD. At the ten-year horizon its median is about $127$ trillion USD, with an interquartile
range of roughly $125$--$130$ and a $5$--$95$th-percentile interval of roughly $121$--$133$; the
...
ensemble members) does not exceed $\pm0.35$ trillion USD for product and $\pm0.03\,^\circ$C for
...
and a tested forward-looking-expectations mechanism (left off by default, as its marginal effect proved
unstable) have all been added. The residual limitations --- the fast-approximation climate block, the
discipline-wide uncertainty of damage estimates, and the intrinsically low predictability of conflict
--- are stated honestly and are not obstacles to applying the model in the modes outlined.
...
This is a channel DICE lacks by construction: across the rate sweep the tension response stays
non-zero (Fig.~\ref{fig:bench-carbon}) --- small and non-monotonic at this scale, but distinct from
DICE's identically zero response.
...
GIM adds the social-tension response DICE cannot represent. Right: the tension response to the tax
rate is non-zero (though small and non-monotonic) against DICE's identically zero response.}
...
mean importer debt-crisis duration rises about $2.5$-fold and four further countries enter sovereign
...
stress rises by about $0.09$ and protest pressure by about $0.013$ on the vulnerable subset
