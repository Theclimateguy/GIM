...
(Table~\ref{tab:compare}, Section~\ref{sec:repo}).
...
\clearpage
\section{Integration benchmark: where sectoral models stop and GIM continues}
\label{app:benchmark}
...
\paragraph{A. Carbon tax (anchor: DICE).} DICE-2016R \citep{nordhaus2017} finds a welfare-optimal
carbon price (about \$31/tCO$_2$ in 2015, rising) whose only first-order effect is lower emissions; it contains no
labour market and no political economy. Fed the same \$50/tCO$_2$ tax, GIM reproduces the emissions
decline (about $6\%$ over ten years) and, in addition, raises social tension and lowers government
trust as the tax's pass-through to energy prices feeds inflation and unemployment --- the
political-economy channel behind the reversals of the French carbon/fuel tax (2018) and the
Australian carbon price (2014). On the dose--response (Fig.~\ref{fig:bench-carbon}) the tension
response rises monotonically with the carbon price, while DICE's response on that axis is zero.

\paragraph{B. Oil price shock (anchor: energy-system models).} An energy-system model such as
MESSAGEix resolves an oil-supply shock as a price rise and demand adjustment, bounded within the
energy sector. GIM carries the same shock into sovereign finance: the higher import bill, financed by
drawing reserves and issuing debt, pushes the most fiscally fragile importers across their
debt-crisis thresholds (which agents tip is decided by the model's own debt and interest-rate
triggers, not selected by hand). At a realistic $-20\%$ supply shock (an import burden of about $3\%$
of GDP) the effect is marginal --- one additional importer in crisis; under a severe shock (about
$15\%$ of GDP, comparable to the worst historical oil episodes) the mean importer debt-crisis
duration rises about $2.7\times$ and four additional importers enter sovereign crisis
(Fig.~\ref{fig:bench-oil}). The response is threshold-stepped, and the energy model produces no
sovereign crises at any magnitude --- the mechanism behind episodes such as Sri Lanka's 2022 default.

\paragraph{C. Crop yield shock (anchor: AgMIP/AR6).} AgMIP and IPCC~AR6 crop assessments stop at
yield decline and a food-security (hunger) index. GIM carries the same yield loss into domestic
instability: cutting the production and reserves of the most income-vulnerable importers opens a
food-affordability gap that raises protest pressure, with convex (accelerating) escalation as the
loss deepens --- the nonlinear ``crossing into high impact'' documented for food prices and the Arab
Spring \citep{lagi2011food} (threshold FAO food-price index $210$). At a severe $50\%$ regional yield
loss, food-affordability stress rises by about $0.09$ and protest pressure by about $0.014$ on the
vulnerable subset (Fig.~\ref{fig:bench-crop}); the chain attenuates toward regime fragility, and the
endpoint is domestic instability, not interstate conflict.

Across the three channels (Fig.~\ref{fig:bench-summary}) GIM exhibits a positive, quantified slope on
...
peak; the dashed line is every sectoral model's structural zero on that axis. GIM's slope is smooth
for the carbon$\to$society channel, threshold-stepped for the oil$\to$finance channel, and convex for
the crop$\to$society channel.}
...
\caption{Scenario~A (carbon tax). GIM reproduces the emissions decline DICE models \emph{(left)} and
adds the social-tension response DICE cannot represent \emph{(middle)}; the dose--response
\emph{(right)} has a non-zero GIM slope against DICE's structural zero.}
...
\caption{Scenario~B (oil price shock). Importer sovereign-debt-crisis duration under a severe shock
\emph{(left)}; the dose--response \emph{(right)} is threshold-stepped, marking the marginal realistic
point and the pronounced severe point, against the energy model's flat zero.}
\label{fig:bench-oil}
\end{figure}
...
\caption{Scenario~C (crop yield shock). Food-affordability stress and protest pressure on the
vulnerable subset \emph{(left)}; the dose--response \emph{(right)} is convex against AgMIP's
structural zero on the protest axis.}
