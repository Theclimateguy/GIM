...
class HistoricalBacktestResult:
    ...
    # [F-price 2026-08-24] Resource prices were never scored against observations: the fixture
    # carried GDP, CO2 and temperature only, so the price subsystem was invisible to every
    # check -- and E6/E13 found the energy price is literally constant across this window while
    # the observed index moves 62%. Defaults keep the stored baseline fixture loadable.
    resource_price_rmse: dict[str, float] = field(default_factory=dict)
    predicted_resource_prices: dict[int, dict[str, float]] = field(default_factory=dict)
    actual_resource_prices: dict[int, dict[str, float]] = field(default_factory=dict)
...
def load_historical_backtest_baseline(
    ...
    return HistoricalBacktestResult(
        ...
        # [F-price 2026-08-24] present in baselines regenerated after prices became a
        # validation target; absent in older ones, hence the default.
        resource_price_rmse={
            str(k): float(v) for k, v in (raw.get("resource_price_rmse") or {}).items()
        },
...
def _run_historical_backtest_member(
    *,
    ...
    # [F-price 2026-08-24] optional so older fixtures still load
    actual_resource_prices: dict[int, dict[str, float]] = {
        int(year): {str(k): float(v) for k, v in values.items()}
        for year, values in (observed.get("resource_price_index_by_year") or {}).items()
    }
    ...
    with _temporary_decarb_rate(decarb_rate_override):
        ...
        predicted_resource_prices: dict[int, dict[str, float]] = {
            start_year: {str(k): float(v) for k, v in world.global_state.prices.items()}
        }
        ...
        for offset in range(1, end_year - start_year + 1):
            ...
            predicted_resource_prices[year] = {
                str(k): float(v) for k, v in world.global_state.prices.items()
            }
    ...
    # [F-price 2026-08-24] Per-resource price RMSE against the renormalised observed index.
    # Empty when the fixture carries no price series, so older fixtures keep working.
    resource_price_rmse: dict[str, float] = {}
    if actual_resource_prices:
        for resource in sorted(next(iter(actual_resource_prices.values())).keys()):
            pairs = [
                (predicted_resource_prices[year].get(resource, float("nan")),
                 actual_resource_prices[year][resource])
                for year in range(start_year, end_year + 1)
                if year in actual_resource_prices and year in predicted_resource_prices
            ]
            pairs = [(p, a) for p, a in pairs if not math.isnan(p)]
            if pairs:
                resource_price_rmse[resource] = _rmse(pairs)
    ...
    return HistoricalBacktestResult(
        ...
        resource_price_rmse=resource_price_rmse,
        predicted_resource_prices=predicted_resource_prices,
        actual_resource_prices=actual_resource_prices,
    )
...
def _aggregate_historical_backtest_results(
    ...
    *,
    ...
    return HistoricalBacktestResult(
        ...
        resource_price_rmse={
            resource: _mean([r.resource_price_rmse[resource] for r in results])
            for resource in anchor.resource_price_rmse
        },
        predicted_resource_prices=anchor.predicted_resource_prices,
        actual_resource_prices=anchor.actual_resource_prices,
    )
