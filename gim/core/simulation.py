...
from .social import (
    apply_structural_trade_balance,
...
from .transitions import (
    ...
    apply_block_layer_substep,
    apply_block_policy_bridge,
...
def _invariant_report(
    ...
    net_exports_sum = 0.0
    net_exports_abs_sum = 0.0
    n_agents_with_trade = 0
    world_gdp = 0.0
    ...
    for agent_id, agent in world.agents.items():
        _ne = float(agent.economy.net_exports)
        net_exports_sum += _ne
        net_exports_abs_sum += abs(_ne)
        n_agents_with_trade += int(_ne != 0.0)
    ...
    # [F-fx 2026-08-24] `abs_share` is a CLOSURE check: it passes trivially when every agent
    # has zero net exports, which is exactly what happens under the scripted policies --
    # net_exports is written only by executed bilateral trade deals (actions.py:404-449) and
    # `simple`/`growth` never propose any. The current account is then identically zero, and
    # the fx-crisis trigger, which reads it, cannot fire at any calibration. `gross` and
    # `n_agents_with_trade` make that inertness visible instead of silent; see
    # Paper/revision/results/e12_fx_trigger_diagnosis.json.
    trade_balance = {
        "net_exports_sum": float(net_exports_sum),
        "net_exports_gross": float(net_exports_abs_sum),
        "n_agents_with_trade": int(n_agents_with_trade),
        "world_gdp": float(world_gdp),
        "abs_share": float(abs(net_exports_sum) / max(world_gdp, 1e-6)),
        "gross_share": float(net_exports_abs_sum / max(world_gdp, 1e-6)),
    }
...
def _run_phase_baseline(
    ...
    # [GIM19 §11 / THE-127] block-model policy bridge refines flagged agents'
    # policy parameters (rate, military share) before policy generation. No-op
    # with BLOCK_LAYER_AGENTS empty.
    apply_block_policy_bridge(world)
...
def _run_phase_propagation(
    ...
    # [F-fx 2026-08-24] Structural resource trade balance -> economy.net_exports, after prices
    # are current and before the financial-crisis checks read the current account. No-op unless
    # STRUCTURAL_TRADE_BALANCE is on.
    apply_structural_trade_balance(world)
    ...
    # [GIM19 §11 / THE-128] Intra-country block layer: social deltas from the
    # fitted block-interaction channel for flagged agents. No-op by default.
    apply_block_layer_substep(world)
