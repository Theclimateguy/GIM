...
from .climate import TGLOBAL_2023_C
    ...
    for agent_id, agent in world.agents.items():
        ...
        for resource_name in RESOURCE_NAMES:
            ...
            if prev_primary is not None:
                desired_base = prev_primary
            elif (resource_name == "metals"
                  and getattr(cal, "METALS_BASE_PRIMARY_NET_OF_RECYCLING", False)):
                # [F-metals 2026-08-24] The base-year `production` loaded from the state CSV is
                # TOTAL market supply -- at the base year it equals consumption. Treating it as
                # PRIMARY and then adding recycled secondary supply on top injects a permanent
                # glut of recycle_rate x consumption in the very first step: measured, metals
                # supply jumps 388.9 -> 565.5 in one year (exactly primary + 0.45 x 392.5),
                # D/S falls to ~0.69 and stays there, and the price walks to its 0.30 floor
                # while the observed index rose 55% over the same window
                # (Paper/revision/results/e13_price_history_validation.json). Netting recycling
                # out of the base primary makes year one supply == the loaded total.
                _rate = max(0.0, min(0.9, metals_recycling_rate))
                desired_base = max(0.0, max(0.0, resource.production)
                                   - _rate * max(0.0, resource.consumption))
            else:
                desired_base = max(0.0, resource.production)

            # [F-food 2026-08-24] Food supply growth. Food production was pinned at its
            # base-year value forever -- measured, world food supply is exactly 1339.3 in every
            # one of 30 forward years -- while food demand grows with population and income
            # (FOOD_DEMAND_POP_ELASTICITY 1.0, INCOME 0.25). Demand/supply therefore climbs to
            # 1.33 by 2053 and the food price to 4.68, against a real record of ~2%/yr
            # production growth (FAO) and roughly FLAT real food prices over decades. Food is
            # also the one resource whose reserve genuinely grows (regen 0.02) while its
            # production does not, which is internally inconsistent.
            # Two terms, both off by default:
            #   FOOD_YIELD_GROWTH        exogenous yield trend, the FAO anchor
            #   FOOD_SUPPLY_PRICE_ELAST  supply response to the price level, which is what
            #                            stabilises real food prices in the observed record
            if resource_name == "food":
                # [F-food-climate 2026-08-24] Temperature -> yield. The model had NO climate
                # channel into food production at all: warming did not touch crops anywhere,
                # which is why Appendix B's crop scenario has to inject a yield cut BY HAND.
                # Prior: Zhao et al. 2017 PNAS 114(35):9326-9331, four independent method
                # families (global gridded models, local point models, statistical regressions,
                # field-warming experiments) converging on a per-degree global yield loss of
                # wheat 6.0%, rice 3.2%, maize 7.4%, soybean 3.1% -- an unweighted mean of
                # ~4.9%/degC, which production weighting toward maize would push toward ~5.5%.
                # Zhao's estimate EXCLUDES CO2 fertilisation, effective adaptation and genetic
                # improvement, so it composes with the price-driven supply response below
                # without double counting: that response IS the model's adaptation mechanism.
                temp_sens = float(getattr(cal, "FOOD_YIELD_TEMP_SENS", 0.0))
                if temp_sens > 0.0:
                    warming = max(0.0, float(world.global_state.temperature_global) - TGLOBAL_2023_C)
                    desired_base *= max(0.0, 1.0 - temp_sens * warming)
                yield_growth = float(getattr(cal, "FOOD_YIELD_GROWTH", 0.0))
                supply_elast = float(getattr(cal, "FOOD_SUPPLY_PRICE_ELAST", 0.0))
                if yield_growth != 0.0:
                    desired_base *= (1.0 + yield_growth)
                if supply_elast > 0.0:
                    food_price = max(1e-6, float(world.global_state.prices.get("food", 1.0)))
                    # level response against the base-year index of 1.0, capped so a runaway
                    # price cannot conjure unbounded output
                    resp = min(float(getattr(cal, "FOOD_SUPPLY_RESPONSE_MAX", 2.0)),
                               food_price ** supply_elast)
                    desired_base *= resp
...
def update_global_resource_prices(
    ...
    for resource_name in RESOURCE_NAMES:
        ...
        if clearing:
            ...
            # [F-buffer 2026-08-24] The damping buffer must be TRADEABLE INVENTORY, not the
            # whole reserve. `global_reserves` mixes two different things: for food it holds
            # about 0.05 years of consumption (~18 days -- genuine carry-over stock, and the
            # reason food is the one resource whose price validates against the World Bank
            # index), but for energy it holds 50 YEARS of consumption, i.e. proven resources in
            # the ground. Those cannot clear a flow imbalance within the year, yet the formula
            # lets them: buffer_ratio -> 1, damped_ratio -> 1, and the energy price is frozen at
            # its initial value for the entire 2015-2023 window while the observed index moves
            # +62%. Capping the buffer at a realistic number of years of consumption leaves
            # food untouched (its reserve is already below the cap) and frees energy and metals.
            # Default cap 0.0 => uncapped, golden bit-identical.
            buffer_years = float(getattr(cal, f"PRICE_BUFFER_YEARS_{resource_name.upper()}", 0.0))
            if buffer_years > 0.0:
                reserve = min(reserve, buffer_years * max(demand[resource_name], epsilon))
