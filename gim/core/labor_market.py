...
_FOOD_PRICE_PREV_ATTR = "_macro_food_price_prev"
...
def update_inflation_unemployment(world: WorldState) -> None:
    ...
    # [F-food 2026-08-24] Food price change, the symmetric cost-push driver. The energy price
    # has had a pass-through into consumer inflation since [E4]; food never did, and E16
    # measured the consequence -- forcing the food price to five times its level moves world
    # GDP, inflation, emissions and social tension by EXACTLY zero to five decimal places. Food
    # was an output-only variable. It is also the LARGER CPI component almost everywhere: food
    # and non-alcoholic beverages carry 8% of the basket in the United States, 12% in the UK,
    # 15.5% in the euro area, 19.5% in Japan, 31% in China and 45% in India, against household
    # energy weights of 3-9.5%. 0.0 => off, golden bit-identical.
    food_price = float(world.global_state.prices.get("food", 1.0))
    food_prev = getattr(world.global_state, _FOOD_PRICE_PREV_ATTR, None)
    if food_prev is None or food_prev <= 0.0:
        food_change = 0.0
    else:
        food_change = (food_price - food_prev) / food_prev
    setattr(world.global_state, _FOOD_PRICE_PREV_ATTR, food_price)
    ...
    for agent in world.agents.values():
        ...
        cost_push += float(getattr(params, "INFLATION_COSTPUSH_FOOD_COEFF", 0.0)) * food_change
        ...
        # [GIM19 B3] Inflation unification: for block-layer agents the block
        # model's CPI (macro bridge, set by the policy bridge in baseline)
        # replaces the core Phillips value — the same getattr-override pattern
        # as _block_policy_rate. Attribute absent (flag off) => bit-identical
        # Phillips path. Okun unemployment above stays the core's.
        block_pi = getattr(econ, "_block_inflation", None)
        if block_pi is not None:
            pi_new = float(block_pi)
