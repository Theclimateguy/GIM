...
def update_capital_endogenous(agent: AgentState, world: WorldState) -> None:
    ...
    if getattr(cal, "CAPITAL_MARKET_CLEARING", False):
        ...
        block_rate = getattr(economy, "_block_policy_rate", None)
        if block_rate is not None:
            # [GIM19 B5] Block-layer credit channel. The generic clearing below
            # anchors gap0 at the first observed gap, so a persistent war-rate
            # LEVEL never reaches investment (only later changes do) — and it
            # compares the real MPK against a NOMINAL rate. For block agents:
            # real cost (nominal − block CPI), anchor at the neutral base, and
            # the literature-range sensitivity BLOCK_CREDIT_SENS.
            real_cost = (float(block_rate)
                         - float(getattr(economy, "_block_inflation", 0.0))
                         + depreciation)
            neutral_cost = cal.BASE_INTEREST_RATE + depreciation
            gap = neutral_cost - real_cost  # >0 when policy looser than neutral
            mult = 1.0 + getattr(cal, "BLOCK_CREDIT_SENS", 1.0) * gap
            # [GIM19 B4] rent → investment: fitted elasticity × real rent
            # growth, computed by the bridge (dynamics_params rent_investment)
            mult += float(getattr(economy, "_block_invest_tilt", 0.0))
        else:
...
def compute_effective_interest_rate(agent: AgentState, world: WorldState | None = None) -> float:
    ...
    # [GIM19 §11 / THE-127] agents with an intra-country block model carry their
    # own fitted policy rate; absent attribute => generic Taylor base as before.
    base_rate = getattr(economy, "_block_policy_rate", base_rate)
...
def update_public_finances(
    ...
    *,
    ...
    # [GIM19 §11 / THE-127] block-model military share when present (fraction of GDP)
    base_military_share = getattr(agent, "_block_military_share", cal.MILITARY_SPEND_BASE)
